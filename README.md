...
5.2版本 支持多设备解锁：任一设备信号达标即可解锁，一轮扫描多目标匹配（功耗与单设备相同）；设备选择页改为多选、首页展示全部设备；解锁成功日志输出命中设备

5.3版本 修复主绑定设备为音频设备时无法自动回连的问题：小米解锁服务会向已登记地址发起 GATT over BR/EDR 连接，抢占音频设备的 BR/EDR ACL，导致耳机跳过 HFP/A2DP 回连（表现为能解锁但没声音）。现改为向系统登记一个固定占位地址避开抢占，解锁判定不受影响；占位地址可通过配置键 `placeholder_mac` 覆盖，置空则回退为登记第一个真实设备的旧行为
